...
#  Sensitivity analysis for direct effects with no 500m boundary
#======================================================================================

run_hTMLE_direct <- function(df,
  ...
  ## select data based on 'dependency'  ---------
  if (!dependency){
    df_overlap = NULL
  }else{
  ...
  # subset to intervention recipients 
  df <- df %>% filter(int_recip == 1)
