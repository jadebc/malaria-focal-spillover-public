...
# 0-21 days for direct effect
# 21-42 days for spillover effect
# ............................................
...
df_all <- df_all %>% 
                mutate(t0 = case_when(target_area == 1 & int_recip == 1 ~ date,
                       ...
                       t1 = case_when(target_area == 1 & int_recip == 1 ~ (date + 21),
                                      target_area == 1 & int_recip == 0 ~ (date + 42),
                                      target_area != 1 ~ (date + 42))
                       )
...
saveRDS(df_all, namibia_df_short_sens_obs_path)
  

## 4. permute intervention at ea level------------------------------------------
set.seed(123)
df_all <- readRDS(namibia_df_short_sens_obs_path)
temp_ea <- df_all %>% dplyr::select(ea, intarm) %>% unique()
temp_ea$intarm <- sample(temp_ea$intarm, size = length(temp_ea$intarm))
df_all_permute <- df_all %>% dplyr::select(-intarm)
df_all_permute <- left_join(df_all_permute, temp_ea, by = "ea") 

# saveRDS(df_all_permute, paste0(namibia_clean_path, "namibia_df_sens_obs_permute_short.RDS"))
