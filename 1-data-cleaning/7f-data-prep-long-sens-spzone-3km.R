...
df_base <- readRDS(namibia_df_base_path_sens_spzone_3km)
...
df_all <- df_all %>% 
  mutate(t0 = case_when(target_area == 1 & int_recip == 1 ~ date,
         ...
         t1 = case_when(target_area == 1 & int_recip == 1 ~ (date + 17),
                        target_area == 1 & int_recip == 0 ~ (date + 180),
                        target_area != 1 ~ (date + 180))
  )
...
saveRDS(df_all, namibia_df_long_sens_spzone_3km_path)
...
# saveRDS(df_all_permute, paste0(namibia_clean_path, "namibia_df_sens_spzone_3km_permute_long.RDS"))
