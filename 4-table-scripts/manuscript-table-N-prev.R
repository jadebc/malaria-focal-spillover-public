...
# Namibia trial
# Table - prevalence primary analysis results
...
make_prev_table <- function(data, reservoir_name, parameter_name, yname){
  ...
  if(reservoir_name=="Human"){
    if(parameter_name=="Direct effect"){
       if(yname == "qPCRposneg")  model <- prev$res_human_de$qPCRposneg$res_est
       if(yname == "rdtyn")  model <- prev$res_human_de$rdtyn$res_est
       if(yname == "hsrdtyn")  model <- prev$res_human_de$hsrdtyn$res_est
    }
    if(parameter_name=="Spillover effect"){
      if(yname == "qPCRposneg")  model <- prev$res_human_sp$qPCRposneg$res_est
      if(yname == "rdtyn")  model <- prev$res_human_sp$rdtyn$res_est
      if(yname == "hsrdtyn")  model <- prev$res_human_sp$hsrdtyn$res_est
    }
    if(parameter_name=="Total effect"){
      if(yname == "qPCRposneg")  model <- prev$res_human_te$qPCRposneg$res_est
      if(yname == "rdtyn")  model <- prev$res_human_te$rdtyn$res_est
      if(yname == "hsrdtyn")  model <- prev$res_human_te$hsrdtyn$res_est
    }
  }
  
  if(reservoir_name=="Mosquito"){
    if(parameter_name=="Direct effect"){
      if(yname == "qPCRposneg")  model <- prev$res_mosq_de$qPCRposneg$res_est
      if(yname == "rdtyn")  model <- prev$res_mosq_de$rdtyn$res_est
      if(yname == "hsrdtyn")  model <- prev$res_mosq_de$hsrdtyn$res_est
    }
    if(parameter_name=="Spillover effect"){
      if(yname == "qPCRposneg")  model <- prev$res_mosq_sp$qPCRposneg$res_est
      if(yname == "rdtyn")  model <- prev$res_mosq_sp$rdtyn$res_est
      if(yname == "hsrdtyn")  model <- prev$res_mosq_sp$hsrdtyn$res_est
    }
    if(parameter_name=="Total effect"){
      if(yname == "qPCRposneg")  model <- prev$res_mosq_te$qPCRposneg$res_est
      if(yname == "rdtyn")  model <- prev$res_mosq_te$rdtyn$res_est
      if(yname == "hsrdtyn")  model <- prev$res_mosq_te$hsrdtyn$res_est
    }
  }
  ...
  if(reservoir_name=="Human & mosquito"){
    if(parameter_name=="Direct effect"){
      if(yname == "qPCRposneg")  model <- prev$res_both_de$qPCRposneg$res_est
      if(yname == "rdtyn")  model <- prev$res_both_de$rdtyn$res_est
      if(yname == "hsrdtyn")  model <- prev$res_both_de$hsrdtyn$res_est
    }
    if(parameter_name=="Spillover effect"){
      if(yname == "qPCRposneg")  model <- prev$res_both_sp$qPCRposneg$res_est
      if(yname == "rdtyn")  model <- prev$res_both_sp$rdtyn$res_est
      if(yname == "hsrdtyn")  model <- prev$res_both_sp$hsrdtyn$res_est
    }
    if(parameter_name=="Total effect"){
      if(yname == "qPCRposneg")  model <- prev$res_both_te$qPCRposneg$res_est
      if(yname == "rdtyn")  model <- prev$res_both_te$rdtyn$res_est
      if(yname == "hsrdtyn")  model <- prev$res_both_te$hsrdtyn$res_est
    }
  }
...
hsrdt_table <- hsrdt_output %>% dplyr::select(-c(PR_unadj, lb_unadj, ub_unadj,
                                                 PR_adj, lb_adj, ub_adj))
# save data for plot
saveRDS(qpcr_output, paste0(namibia_process_path, "plot-data-prev-primary-qpcr.RDS"))

# save tables ---------------------------------------
write.csv(qpcr_table, paste0(table_path, "table-prev-primary-qpcr.csv"))
write.csv(rdt_table, paste0(table_path, "table-prev-primary-rdt.csv"))
write.csv(hsrdt_table, paste0(table_path, "table-prev-primary-hsrdt.csv"))
